...
func decode(jpg string) ([][][]float32, error) {
	...
	m := image.NewYCbCr(image.Rect(0, 0, 227, 227), 0)
	if err := rez.Convert(m, img, rez.NewBicubicFilter()); err != nil {
		return nil, err
	}

	result := make([][][]float32, 3)
	result[0] = make([][]float32, 227)
	result[1] = make([][]float32, 227)
	result[2] = make([][]float32, 227)
	for row := range result[0] {
		result[0][row] = make([]float32, 227)
		result[1][row] = make([]float32, 227)
		result[2][row] = make([]float32, 227)
		for col := range result[0][row] {
			c := m.At(col, row).(color.YCbCr)
			r, g, b := color.YCbCrToRGB(c.Y, c.Cb, c.Cr)
			// squeezenet expects BGR
			result[0][row][col] = float32(b)
			result[1][row][col] = float32(g)
			result[2][row][col] = float32(r)
		}
	}
	return result, nil
}
